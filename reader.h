...
class Reader
{
    ...
    void parseValue(const std::string &strVal, std::vector<std::string> &val)const; // make array of strings
    void parseValue(const std::string &strVal, size_t &val) const; // size_t conversion
    void parseValue(const std::string &strVal, unsigned int &val) const; // 32 bit unsigned int conversion

    template <class T >  void parseValue(const std::string &strVal, std::vector<T> &val) const
    {
        /*!
...
void Reader::parseValue(const std::string &strVal, std::vector<std::string> &val) const{
    ...
    if ( (i1 = str.find_first_of(_R_OBRACE)) != str.find_last_of(_R_OBRACE)) {throw _R_VECTOR_FORMAT_ERROR_MSG;} // multiple '['
    if ( (i2 = str.find_first_of(_R_CBRACE)) != str.find_last_of(_R_CBRACE)) {throw _R_VECTOR_FORMAT_ERROR_MSG;} // multiple ']'
    if (i1 > 0) {throw _R_VECTOR_FORMAT_ERROR_MSG;} //  '[' not first
    if (i2 < str.length()-1) {throw _R_VECTOR_FORMAT_ERROR_MSG;} //']' not last
    if (i2 == std::string::npos) {throw _R_VECTOR_FORMAT_ERROR_MSG;} // no closing ']'
    if (i1 == i2-1) {throw _R_VECTOR_FORMAT_ERROR_MSG;} // empty vector
